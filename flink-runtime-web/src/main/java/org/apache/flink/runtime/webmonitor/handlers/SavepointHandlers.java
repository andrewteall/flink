...
/**
 * Request handler for {@link TriggerSavepoint} messages.
 */
public class SavepointHandlers {

	/** URL for in-progress cancellations. */
	public static final String IN_PROGRESS_URL = "/jobs/:jobid/savepoint/in-progress/:requestId";
	...
	/**
	 * Handler for triggering a {@link TriggerSavepoint} message.
	 */
		...
		private FullHttpResponse handleNewRequest(ActorGateway jobManager, final JobID jobId, String targetDirectory, long checkpointTimeout) throws IOException {
			...
			if (isNewRequest) {
				...
				try {
					...
					Object msg = new TriggerSavepoint(jobId, Option.apply(targetDirectory));
					...
					cancelFuture.onComplete(new OnComplete<Object>() {
						...
						public void onComplete(Throwable failure, Object resp) throws Throwable {
							synchronized (lock) {
								try {
									if (resp != null) {
										if (resp.getClass() == TriggerSavepointSuccess.class) {
											String path = ((TriggerSavepointSuccess) resp).savepointPath();
											completed.put(requestId, path);
										} else if (resp.getClass() == TriggerSavepointFailure.class) {
											Throwable cause = ((TriggerSavepointFailure) resp).cause();
